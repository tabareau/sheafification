...
\begin{abstract}
  ...
  mismatch has been fixed recently by considering homotopy type
...
Type theory is known to be quite close to topos theory so one could
...
We remark that axioms for a Lawvere-Tierney topology on the subobject
...
The first thing we can note is that such a construction will not allow to reach every type:
...
rest of the section, we fix one, denoted $\modal_{-1}$. The reader can
